...
In the case of insertion, we are going to insert jth character of s2 into s1 so cost_ins + dp[i][j-1]... ( dp[i][j-1] would be cost before inserting jth character ) 
In the case of deletion, we are going to delete ith character of s1 so cost_del + dp[i-1][j]... 	( dp[i-1][j] would be cost before deleting ith character )
...
Here all the costs are taken 1. We can change the values of them if it is mentioned in the problem.
